...
function Refund_Income() {
  ...
  const Binary_Income_Api = async () => {
    try {
      let responce = await API.get(
        `/GetRefundIncome2?loginuid=${uId}&uid=${userid}&amount=${packageAmount}`
      );
      ...
      responce.forEach((item, index) => {
        arr.push({
          ...
          date: `${item.edate1}`,
          onamount: item.amount,
          remark: `${item.transtype} `,
        });
      });
  ...
  var [matching_income, set_matching_income] = new useState({
    cols: [
      ...
      { Header: "Package Amount", accessor: "onamount" },
