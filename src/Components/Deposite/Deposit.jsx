...
function Deposit() {
  ...
  const USDTBalance = async () => {
    try {
      ...
      result = parseFloat(result) / 1000000; //window.web3.utils.fromWei(result);
      console.log("result-->", result);
      setbalanceUle(result.toFixed(2));
    } catch (error) {
      setloading(false);
      console.log(error);
    }
      ...
      if (Wallet_Address.toUpperCase() == acc.toUpperCase())
        if (amount != 0) {
          if (balanceUle < amount) {
            ...
            let amountValue = parseFloat(amount) * 1000000; //window.web3.utils.toWei(amount.toString());
