...
function IncomeToActivation() {
  ...
  const SendOtp_API = async (value) => {
    ...
    try {
      ...
      let res = await API.post("/SendOTPforIncomeFundConvertAPI", {
        uid: value,
      });
      res = res.data.data;
    ...
    try {
      ...
      try {
        let responce = await API.post(
          `/Incomefund_Converto_ActivationFundAPI`,
          {
            uid: uid,
            pswddd: "",
            touser_id: TransId,
        ...
        if (responce == "SUCCESSFUL") {
          toast("Icome Fund To Activation Fund Convert Successfully", {
            className: "copied-message",
          });
  ...
  const Binary_Income = async () => {
    try {
      let responce = await API.get(
        `/ActivationWalletstatementUser?uid=${userid}`
      );
      responce = responce.data.data;
      ...
      <div className="main-content app-content">
        ...
        <div className="main-container container-fluid">
          ...
          <div className="breadcrumb-header justify-content-between">
            <div className="left-content">
              <span className="main-content-title mg-b-0 mg-b-lg-1">
                Convert Income Fund To Activation Fund History
              </span>
            </div>
          </div>
          {/* <!-- /breadcrumb --> */}
...
export default IncomeToActivation;
